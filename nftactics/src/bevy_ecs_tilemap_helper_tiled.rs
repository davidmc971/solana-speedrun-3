// ! You can find this file in the bevy_ecs_tilemap repo at:
// ! https://github.com/StarArawn/bevy_ecs_tilemap/blob/e4f3cc668110d9a46f6fe407db4f6d4daeabc3d2/examples/helpers/tiled.rs
// ! It is licensed under the MIT license and modified slightly to fit our needs.
...
impl AssetLoader for TiledLoader {
    ...
    fn load<'a>(
        ...
        Box::pin(async move {
            ...
            let map = loader.load_tmx_map("assets/tiled/map_0.tmx").unwrap();
            ...
            for (tileset_index, tileset) in map.tilesets().iter().enumerate() {
                let tilemap_texture = match &tileset.image {
                    ...
                    Some(img) => {
                        // let image_source = &img.source;
                        // let image_source_str = image_source.to_str().unwrap();
                        // let mut image_source_split = image_source_str.split("..");
                        // let image_source_after_dotdot = image_source_split.nth(1).unwrap();
                        // let image_source_after_dotdot_noslash = &image_source_after_dotdot[1..];
                        // let tile_path = Path::new(image_source_after_dotdot_noslash);
                        // let asset_path = AssetPath::from(tile_path);
                        // let texture: Handle<Image> = load_context.load(asset_path.clone());

                        // Sad Pepe when seeing this code I wrote :^) ~ David
                        ...
                        let tmx_dir = load_context
                            ...
                            .expect("The asset load context was empty.");
                        let tile_path = tmx_dir.join(&img.source);
                        let tile_path_str = tile_path.to_str().unwrap().to_string();
                        let tile_path_split: Vec<String> =
                            tile_path_str.split("..").map(|s| s.to_string()).collect();
                        let tile_path_after_dotdot = tile_path_split.get(1).unwrap().clone();
                        let tile_path_after_dotdot_noslash = tile_path_after_dotdot
                            .strip_prefix('/')
                            .unwrap()
                            .to_string();
                        let tile_path_after_dotdot_noslash_cloned =
                            tile_path_after_dotdot_noslash.clone();
                        let asset_path =
                            AssetPath::from(tile_path_after_dotdot_noslash_cloned).to_owned();
                        let texture: Handle<Image> = load_context.load(asset_path.clone());
                        ...
                        let tiled::LayerType::Tiles(tile_layer) = layer.layer_type() else {
                            match layer.layer_type() {
                                tiled::LayerType::Objects(_) => {
                                    ...
                                    // Handle object layer
                                    log::info!(
                                        "Object Layer found with id {} and name {}",
                                        layer.id(),
                                        layer_name
                                    );
                                    let layer = layer.as_object_layer().unwrap();
                                    for obj in layer.objects() {
                                        log::info!(
                                            "Found object with id {} and name {}",
                                            obj.id(),
                                            obj.name
                                        );
                                        match (layer_name, obj.name.as_str()) {
                                            ("colliders", _) => {
                                                log::info!(
                                                    "Found collider object with id {} and name {}",
                                                    obj.id(),
                                                    obj.name
                                                );
                                            }
                                            _ => {
                                                log::info!(
                                                    "Skipping object with id {} and name {} because it is not a supported object type.",
                                                    obj.id(),
                                                    obj.name
                                                );
                                                continue;
                                ...
                                _ => {
                                    log::info!(
                                        "Skipping layer {} because {:?} is not a supported layer type.",
                                        layer.id(),
                                        layer.layer_type()
                                    );
                                    continue;
